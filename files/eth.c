...
eth_iface_t * eth_open ( char* ifname )
{
  ...
  if (raw_iface == NULL) {
    print_alert("eth.c: eth_open(): raw_iface = null\n");
    fprintf(stderr, "\tERROR en rawiface_open(): %s\n",
            rawnet_strerror());
    return NULL;
  }  
  ...
  mac_addr_t dst, uint16_t type, unsigned char * payload, int payload_len )
{
 ...
 /* Ain't nobody got tiem 2 prin' dis
  ...
  mac_addr_t src, uint16_t type, unsigned char buffer[], long int timeout )
{
  ...
  do {
    //print_success ("WE ARE IN DO LOOP\n");
...
is_my_mac = (memcmp(eth_frame_ptr->dest_addr, 
  ...
  //} while ( ! (is_my_mac && is_target_type) );
   }while (!mac_subscribed_to (iface, eth_frame_ptr->dest_addr) && !is_target_type );
...
int mac_subscribed_to( eth_iface_t * iface, mac_addr_t addr ){
  
mac_addr_t rip_multicast;
mac_str_addr( "01:00:5E:00:00:09",rip_multicast );

if(memcmp(addr, iface->mac_address, MAC_ADDR_SIZE) == 0) return 1;
if(memcmp(rip_multicast, iface->mac_address, MAC_ADDR_SIZE) == 0) return 1;

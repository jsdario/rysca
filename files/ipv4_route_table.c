...
int ipv4_route_lookup ( ipv4_route_t * route, ipv4_addr_t addr )
{
  ...
  //TO STORE ROUTE IPv4 ADDR AND MASK
  ...
  //BUFFER CONTAINING MASK
  char aux_mask_buffer [IPv4_STR_SIZE];
  
  //AUX IN UNSIGNED INT FORMAT
  unsigned int aux_int [IPv4_ADDR_SIZE];

  //AUX MASK IN UNSIGNED INT FORMAT
  unsigned int aux_mask_int [IPv4_ADDR_SIZE];

  //ADDR IN UNSIGNED INT FORMAT
  ...
  for (i=0; i<IPv4_ADDR_SIZE; i++){
    ...
    if (addr_int [i] != aux_int [i]) flag = 1;
